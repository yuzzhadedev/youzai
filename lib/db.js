...
export async function findAuthUserByEmail(email = '') {
  ...
  const supabase = await getSupabase();
  if (!supabase) return localStore.auth_users?.[normalizedEmail] || null;
  ...
  return localStore.auth_users?.[normalizedEmail] || null;
}
...
export async function saveAuthUser({ name, email, passwordHash, picture = '', provider = 'email' }) {
  ...
  const supabase = await getSupabase();
  if (!supabase) {
    localStore.auth_users[normalizedEmail] = row;
    persistStore();
    return row;
  }
...
export async function getUserSettings(userKey = '') {
  ...
  const supabase = await getSupabase();
  if (!supabase) {
    return localStore.user_settings?.[key] || null;
  }
...
export async function upsertUserSettings(userKey = '', nextSettings = {}) {
  ...
  const supabase = await getSupabase();
  if (!supabase) {
    localStore.user_settings[key] = merged;
    persistStore();
    return merged;
  }
...
export async function saveConversationMessage({ conversationId, role, content }) {
  const supabase = await getSupabase();
  if (!supabase || !conversationId || !isUuid(conversationId)) return;
...
export async function listNotifications(userKey = '', { onlyUnread = false } = {}) {
  ...
  const supabase = await getSupabase();
  if (!supabase) {
    let rows = (localStore.notifications || []).filter((row) => row?.target === 'all' || row?.target_user_key === key);
    if (onlyUnread) rows = rows.filter((row) => !row?.read_by?.includes(key));
...
export async function pushNotification({ admin = 'admin', target = 'all', targetUserKey = '', title = '', message = '' }) {
  ...
  const supabase = await getSupabase();
  if (!supabase) {
    localStore.notifications = Array.isArray(localStore.notifications) ? localStore.notifications : [];
    localStore.notifications.unshift(row);
    localStore.notifications = localStore.notifications.slice(0, 5000);
    persistStore();
    return row;
  }
...
export async function markNotificationsRead(userKey = '', ids = []) {
  ...
  if (!supabase) {
    let updated = 0;
    localStore.notifications = (localStore.notifications || []).map((row) => {
      if (idSet.size && !idSet.has(String(row?.id))) return row;
      const rb = Array.isArray(row?.read_by) ? row.read_by : [];
      if (!rb.includes(key)) { rb.push(key); updated++; }
      return { ...row, read_by: rb };
    });
