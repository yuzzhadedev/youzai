...
async function getPremiumRow(userKey) {
  ...
  if (!supabase) {
    for (const key of candidates) {
      if (localStore.premium_users[key]) return localStore.premium_users[key];
    }
    return null;
  }
  ...
  const queryByColumns = async (column) => {
    const values = column === 'email' ? emailCandidates : candidates;
  ...
  const userKeyResult = await queryByColumns('user_key');
  if (!userKeyResult.error && Array.isArray(userKeyResult.data) && userKeyResult.data.length > 0) {
    return userKeyResult.data[0];
  }

  const emailResult = await queryByColumns('email');
  if (!emailResult.error && Array.isArray(emailResult.data) && emailResult.data.length > 0) {
  ...
  for (const key of emailCandidates) {
    if (localStore.premium_users[key]) return localStore.premium_users[key];
    if (localStore.premium_users[`email:${key}`]) return localStore.premium_users[`email:${key}`];
  }
...
export async function getUserPlan(userKey) {
  ...
  const isPremiumPlan = planName === 'premium' || planName.startsWith('premium_') || planName.startsWith('premium-');
