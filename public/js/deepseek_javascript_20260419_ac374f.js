        ...
        chatMessages.innerHTML = `
            <div class="empty-state">
                ...
                <div class="suggestions">
                    ...
                    <button class="suggestion-btn" data-prompt="🤖 Apa itu kecerdasan buatan?"><i class="fas fa-brain"></i> Apa itu AI?</button>
                </div>
            </div>`;
...
messageInput.addEventListener('input', function() {
    ...
    if (query.length > 2) {
        ...
        if (items.length > 0) {
            items[0].setAttribute('data-query', `${query} berita terbaru`);
            items[0].innerHTML = `<i class="fas fa-newspaper"></i><span>"${query}" berita terbaru</span>`;
            items[1].setAttribute('data-query', `${query} penjelasan`);
            items[1].innerHTML = `<i class="fas fa-search"></i><span>"${query}" penjelasan</span>`;
        }
    } else { searchSuggestions.classList.add('hidden'); }
});

document.querySelectorAll('.suggestion-item').forEach(item => {
    item.addEventListener('click', () => { messageInput.value = item.dataset.query; searchSuggestions.classList.add('hidden'); sendMessage(); });
});
...
document.addEventListener('click', (e) => { if (window.innerWidth <= 768 && !sidebar.contains(e.target) && !hamburgerBtn.contains(e.target) && !sidebar.classList.contains('closed')) sidebar.classList.add('closed'); });
window.addEventListener('resize', () => { if (window.innerWidth > 768) sidebar.classList.remove('closed'); });
