...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    try {
      ...
      return res.status(200).json({ success: false, content: error?.message || 'Gagal memuat info premium.' });
    }
  }
  ...
  if (req.method === 'POST') {
    ...
    if (action === 'confirm') {
      try {
        ...
        return res.status(200).json({ success: false, content: error?.message || 'Gagal mengaktifkan premium.' });
      }
    }
    ...
    try {
      ...
      return res.status(200).json({ success: false, content: error?.message || 'Gagal membuat permintaan premium.' });
    }
  }
